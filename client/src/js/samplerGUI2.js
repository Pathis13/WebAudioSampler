import { SamplerEngine } from './samplerEngine2.js';
    ...
    bindButtonEvents(){
        ...
        for (let item of buttons) {
            ...
            item.addEventListener('click', () => {
                this.engine.playSound(this.sounds[item.id])
                this.context.clearRect(0, 0, this.canvas.width, this.canvas.height);
                this.sounds[item.id].waveForm.drawWave(0, this.canvas.height);
                this.activeSoundIndex = item.id;
                this.mouseEvents()
            });
        }
    }
